#!/bin/bash
# Script to easily create a ecc_theme sub-theme.
...
if [ "${foldername}" != "ecc_theme" ]; then
  echo 'Error: This command should be run from the LocalGov Base Theme root folder (themes/custom/ecc_theme).'
  exit
fi

if [ "${parentfoldername}" != "custom" ]; then
  echo 'Error: The ecc_theme theme (this folder) should be in the "custom" folder.'
  exit
fi
...
cp ../../custom/ecc_theme/logo.svg .
echo "+ themes/custom/$LGD_SUB_THEME/logo.svg copied"

cp ../../custom/ecc_theme/scripts/subtheme-items/_subtheme.info.yml_ $LGD_SUB_THEME.info.yml
echo "+ themes/custom/$LGD_SUB_THEME/$LGD_SUB_THEME.info.yml created"
cp ../../custom/ecc_theme/scripts/subtheme-items/subtheme.libraries.yml $LGD_SUB_THEME.libraries.yml
echo "+ themes/custom/$LGD_SUB_THEME/$LGD_SUB_THEME.libraries.yml created"
cp ../../custom/ecc_theme/scripts/subtheme-items/subtheme.theme $LGD_SUB_THEME.theme
echo "+ themes/custom/$LGD_SUB_THEME/$LGD_SUB_THEME.theme created"
cp ../../custom/ecc_theme/scripts/subtheme-items/package.json package.json
echo "+ themes/custom/$LGD_SUB_THEME/package.json created"
cp ../../custom/ecc_theme/scripts/subtheme-items/.nvmrc .nvmrc
echo "+ themes/custom/$LGD_SUB_THEME/.nvmrc created"
cp ../../custom/ecc_theme/scripts/subtheme-items/.stylelintrc.json .stylelintrc.json
...
cp -r ../../custom/ecc_theme/scripts/subtheme-items/.gitignore .
echo "+ .gitignore copied"

cp -r ../../custom/ecc_theme/scripts/subtheme-items/css .
echo "+ default css copied"

cp -r ../../custom/ecc_theme/scripts/subtheme-items/templates .
...
cp -r ../../custom/ecc_theme/scripts/subtheme-items/assets .
echo "+ added default favicons - you will probably want to create new ones for yourself."

echo ""
echo "# Your new ecc_theme sub-theme has been created."
